...
# Reduce background parallelism to lower memory/threads contention
...
def get_process_pool():
    try:
        return conc_fut.ProcessPoolExecutor(max_workers=1, mp_context=get_context("spawn"))
    except Exception:
        return conc_fut.ProcessPoolExecutor(max_workers=1)

def _hf_generate_worker(model_path: str, offline: bool, prompt: str, max_new_tokens: int, num_beams: int) -> str:
    from transformers import AutoTokenizer, AutoModelForCausalLM, pipeline
    tok = AutoTokenizer.from_pretrained(model_path, local_files_only=offline)
    mdl = AutoModelForCausalLM.from_pretrained(model_path, local_files_only=offline, low_cpu_mem_usage=True, device_map="cpu")
    gen = pipeline("text-generation", model=mdl, tokenizer=tok, return_full_text=False)
    out = gen(prompt, max_new_tokens=int(max_new_tokens), do_sample=False, num_beams=int(num_beams))
    return out[0].get('generated_text','').strip()

def _gguf_generate_worker(model_path: str, n_ctx: int, n_batch: int, prompt: str, max_new_tokens: int) -> str:
    from llama_cpp import Llama
    llm = Llama(model_path=model_path, n_ctx=int(n_ctx), n_batch=int(n_batch), use_mmap=True, use_mlock=False)
    out = llm.create_completion(prompt=prompt, max_tokens=int(max_new_tokens), temperature=0.2, top_p=0.9)
...
model_dir = "C:/Users/merve/models/llS"
llm_models = [os.path.join(model_dir, f) for f in os.listdir(model_dir) if f.endswith(".gguf")]
...
with st.sidebar:
    ...
    gen_timeout = st.select_slider("LLM timeout (sn)", options=[10, 20, 30, 45, 60], value=30)
    run_in_subprocess = st.checkbox("LLM'i ayrı süreçte çalıştır (önerilir)", value=True)
...
if uploaded is not None:
    ...
    try:
        ...
        st.dataframe(summary, width='stretch', height=240)
                        ...
                        if llm_backend == "HF":
                            try:
                                ...
                                if run_in_subprocess:
                                    ...
                                    fut = pool.submit(_hf_generate_worker, active_llm, llm_offline, prompt, int(gen_max_new), int(gen_beams))
                                    text = fut.result(timeout=int(gen_timeout))
                                else:
                                ...
                                st.warning(f"HF LLM hata/timeout: {e}. Kural tabanlı öneriye düşüyorum.")
                            ...
                            try:
                                ...
                                if run_in_subprocess:
                                    pool = get_process_pool()
                                    fut = pool.submit(_gguf_generate_worker, active_llm, int(gguf_ctx), int(gguf_batch), prompt, int(gen_max_new))
                                    text = fut.result(timeout=int(gen_timeout))
                                else:
                                ...
                                st.warning(f"GGUF LLM hata/timeout: {e}. Kural tabanlı öneriye düşüyorum.")
                                suggestion = rule_based_improvement(req_text, miss)
                                st.text_area("AI Önerisi", value=suggestion, height=200)
                                st.stop()
